...
process_request(?CONNECT,
    ...
    rabbit_log:info("ZH: MQTT ON CONNECT~n"),
    {ReturnCode, PState1} =
...
dbLogin(User, Pass) ->
    rabbit_log:info("ZH: check mongodb~n"),

    application:start(bson),
    application:start (mongodb),
    Host = {"db.onion.io", 27017},
    {ok, Conn} = mongo:connect(Host),
    {ok, Result} = mongo:do (safe, master, Conn, onion, fun() ->
        mongo:auth (<<"onion">>, <<"!<684ygrJ51Vx)3">>),
        mongo:find_one (devices, {id,list_to_binary(User),key,list_to_binary(Pass)})
                                         end),
    case Result of
        {} ->
            rabbit_log:info("ZH: login fail~n"),
            false;
        _ ->
            rabbit_log:info("ZH: login pass~n"),
            true
    end.
...
creds(User, Pass) ->
    ...
    Anon        = false,
    case dbLogin(User, Pass) of 
        true -> {DefaultUser, DefaultPass};
        false -> nocreds
    end.
